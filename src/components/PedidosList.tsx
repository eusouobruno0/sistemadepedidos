                ...
                return (
                  <div key={p.id} className="p-4 space-y-3 bg-white hover:bg-slate-50/60 transition-colors">
                    ...
                    <div className="flex items-center justify-between gap-2 flex-wrap">
                      ...
                      {/* Alternância da Situação Comercial */}
                      <button
                        type="button"
                        onClick={() => {
                        ...
                        className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold transition-all shadow-2xs cursor-pointer border ${
                          situacao === 'Fechado'
                    ...
                    <div>
                      ...
                      {ocCliente && (
                        <div className="text-xs font-mono text-slate-700 mt-1 flex items-center gap-1">
                          <span className="font-bold">OC:</span>
                          <span className="bg-slate-100 text-indigo-900 font-bold px-1.5 py-0.5 rounded border border-slate-200">
                            {ocCliente}
                          </span>
                        </div>
                      )}
                      {p.numeroPedidoIndustria && (
                        <div className="text-xs font-mono text-slate-600 mt-0.5">
                          <span className="font-semibold">Nº Indústria:</span> {p.numeroPedidoIndustria}
                        </div>
                      )}
                    </div>

                    {/* Valores do Pedido com destaque */}
                    <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 grid grid-cols-2 gap-2 text-xs">
                      <div>
                        <span className="text-[11px] text-slate-500 block">Subtotal Itens:</span>
                        <span className="font-mono font-bold text-slate-800">{formatCurrency(p.totalItens)}</span>
                      </div>
                      <div>
                        <span className="text-[11px] text-amber-700 block">Total IPI:</span>
                        <span className="font-mono font-bold text-amber-800">{formatCurrency(p.totalIpi)}</span>
                      </div>
                      <div className="col-span-2 pt-1 border-t border-slate-200 flex items-center justify-between">
                        <span className="font-bold text-slate-700 text-xs uppercase">Valor Total:</span>
                        <span className="font-mono font-black text-base text-emerald-800">{formatCurrency(p.totalPedido)}</span>
                      </div>
                    </div>
                    ...
                    {/* Barra de Ações com botões grandes e confortáveis */}
